...
def main():
    ...
    machine.AddCircuit(type='RCLP', name='rclp',
                       fcut=100, order=1, pushed=True)
    # amplitude detector for the filter
    machine.AddCircuit(type='minmax', name='arclp', CheckTime=0.2, pushed=True)

    # passive high pass filter
    machine.AddCircuit(type='RCHP', name='rchp',
                       fcut=100, order=1, pushed=True)
    # amplitude detector for the filter
    machine.AddCircuit(type='minmax', name='archp', CheckTime=0.2, pushed=True)

    # connect oscillator to the filters
    machine.Connect("wave.sin", "sklp.signal", "skhp.signal",
                    "skbp.signal", "rclp.signal", "rchp.signal")
    ...
    out1 = machine.AddCircuit(
        type='output', name='output', file='test_filters.out', dump=0)
    out1.Register('wave.freq', 'asklp.amp', 'askhp.amp',
                  'askbp.amp', "arclp.amp", "archp.amp")
