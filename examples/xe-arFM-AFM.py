...
def main():
	...
	machine = Machine(machine=None, name='machine', dt=1e-7)
	...
	machine.AddCircuit(type="PI",name='agc', Ki=4, Kp=400, set=Az, pushed=True)
	machine.AddCircuit(type="limiter",name='agclim', min=0,max=10, pushed=True)
	
	machine.AddCircuit(type="Machine",name='pll', assembly=dPFD, gain=600.0, fcut=500, KI=300,KP=0.3, f0=f0	, pushed=True)
