...
def main():
    ...
    # Add Circuits
    canti = machine.AddCircuit(type='AdvancedCantilever', name='canti',
                               NumberOfModesV=2, NumberOfModesL=1, pushed=True)
    scanner = machine.AddCircuit(type='Scanner', name='scan', pushed=True)

    machine.AddCircuit(type='waver', name='wave', amp=10,
                       freq=1, phi=1, offset=0, pushed=True)
...
#	machine.Connect("wave.cos","canti.ForceL")
    ...
    out1 = machine.AddCircuit(
        type='output', name='output', file='AdvCantilever.dat', dump=1)
