...
#		Q=20000, k=26.4, f0=150000, startingz=1, pushed=True)
    ...
    machine.AddCircuit(type="Machine", name='amp', fcut=5000, assembly=aAMPD,
                       pushed=True)
    machine.AddCircuit(type='PI', name='agc', Kp=1.0,
                       Ki=100, set=A0, pushed=True)
    ...
    inter.Configure(steps=[0.0705, 0.0705, 0.01], npoints=[
                    8, 8, 201])  # I converted it back to nN
...
# Outputs
    ...
    out2 = machine.AddCircuit(
        type='output', name='output2', file='testafm2.out', dump=100000)
    out2.Register('global.time', 'amp.amp', 'agc.out', 'pll.df',
                  "canti.fz", 'scan.z')  # always monitor the amplitude
