...
class Scanner(Circuit):
    ...
    def __init__(self, machine, name, **keys):
        ...
        Circuit.cCore.Scanner_Place.argtypes = [
    ...
    def Move(self, x=0, y=0, z=0, v=1):  # default arguments, make the input lighter
        ...
        steps = Circuit.cCore.Scanner_Move(self.cCoreID, c_double(
            x), c_double(y), c_double(z), c_double(v))
    ...
    def Place(self, **kw):  # all parameters required
        ...
        steps = Circuit.cCore.Scanner_Place(
            self.cCoreID, c_double(x), c_double(y), c_double(z))
    ...
    def MoveTo(self, **kw):
        ...
        steps = Circuit.cCore.Scanner_MoveTo(
            self.cCoreID, c_double(x), c_double(y), c_double(z), c_double(v))
        self.machine.main.WaitSteps(steps)
    ...
    def ScanArea(self):
        ...
        for i in range(3):
            ...
            dslow[i] = self._slowscan[i] * \
                self.ImageSize[1] / float(self.Resolution[1])
    ...
    def MoveRecord(self, **kw):
        ...
        if ("points" in list(kw.keys())):
            ...
            raise NameError(
                "ERROR! Scanner MoveRecord requires number of points.")

        Circuit.cCore.Scanner_Move_Record.restype = c_ulonglong
        steps = Circuit.cCore.Scanner_Move_Record(self.cCoreID, c_double(
            x), c_double(y), c_double(z), c_double(v), c_int(npts))
        self.machine.main.WaitSteps(steps)
        print("Scanner moved by " + str(x) + "," + str(y) + "," + str(z))
    ...
    def SinScan(self, **kw):
        ...
        steps = Circuit.cCore.SinScan(
            self.cCoreID, c_double(freq), c_double(amp), c_int(cycle))
...
class CoordTransform(Circuit):
    ...
    def __init__(self, machine, name, **keys):
        ...
        Circuit.cCore.CoordTransform.argtypes = [
            c_int, c_double, c_double, c_double, c_double, c_double, c_double, c_double, c_double, c_double]

        self.cCoreID = Circuit.cCore.CoordTransform(self.machine.cCoreID, self.LatticeVectorX[0], self.LatticeVectorX[1], self.LatticeVectorX[2], self.LatticeVectorY[
                                                    0], self.LatticeVectorY[1], self.LatticeVectorY[2], self.LatticeVectorZ[0], self.LatticeVectorZ[1], self.LatticeVectorZ[2])
