...
import vafmcircuits_output
import vafmcircuits_signal_gens
...
class Machine(Circuit):
    ...
    def __init__(self, machine=None, name="machine", **keys):
        ...
        if(Machine.singleton == False and machine == None):
            ...
            if isMain == 1:
                ...
                print(
                    "For complete licence details please vist: http://singroup.github.io/PyVAFM/page_Licence.html")
                print("       ")
        ...
        if not ('dt' in list(keys.keys())):
            if(machine != None):
                ...
                print(
                    'WARNING! No timestep dt was given in the initialisation parameters.')
        else:
            self.dt = keys['dt']
    ...
    def SetInputs_fromKeys(self, **kwargs):
        ...
        for key in list(kwargs.keys()):
            ...
            if key in list(self.I.keys()):
                ...
                # deprecated... sets the value in python!
                self.I[key].Set(kwargs[key])

                # find the position of the key
                idx = list(self.I.keys()).index(key)
                print("PY: "+key+" is an input channel, calling cCore(c):",
                      idx, c_double(kwargs[key]))

                Circuit.cCore.SetContainerInput(
                    self.cCoreID, idx, c_double(kwargs[key]))
    ...
    def AddInput(self, name):
        ...
        if name in list(self.I.keys()) or name in list(self.O.keys()):
            raise NameError("A channel named "+name +
                            " already exists in composite circuit " + str(self))

        self.I[name] = Channel(name, self, True)
        self._MetaI[name] = Channel(name, self, False)

        # add the channel also on the cCore
        self.cCoreI.append(Circuit.cCore.Add_ChannelToContainer(
            self.cCoreID, 1))  # 1 for input
    ...
    def AddOutput(self, name):
        ...
        # add the channel also on the cCore
        self.cCoreO.append(Circuit.cCore.Add_ChannelToContainer(
            self.cCoreID, 0))  # 1 for input
    ...
    def AddCircuit(self, **argkw):
        ...
        for i in range(len(lst)):  # loop over the modules
            ...
            # if the module is a vafmcircuits module
            if lst[i].startswith('vafmcircuits'):

                # extract classes from the module, except the abstract one
                c = [s for s in inspect.getmembers(
                    sys.modules[lst[i]], inspect.isclass) if s[0] == ctype]
        ...
        if classobj == None:
            raise NotImplementedError(
                "Circuit "+ctype+" was not implemented or imported!")

        # check if the name was good
        if cname in list(self.circuits.keys()):
            raise NameError("A circuit named '"+cname +
                            "' already exists in the setup!")
    ...
    def _GetInternalChannel(self, tag, chtype=ChannelType.Any):
        ...
        if len(ctag) != 2:
            raise SyntaxError(
                "GetInternalChannel error: channel tag "+tag + " is invalid.")
        ...
        if cname == 'global':
            ...
            raise SyntaxError(
                "GetInternalChannel error: the VAFM can only perform connections between internal circuits")

        # check the name of the circuit
        if not(cname in list(self.circuits.keys())):
            raise NameError(
                "GetInternalChannel error: circuit "+cname+" not found.")
    ...
    def _GetMetaChannel(self, tag, chtype=ChannelType.Any):
        ...
        if len(ctag) != 2:
            raise SyntaxError(
                "GetMetaChannel error: channel tag "+tag + " is invalid.")
    ...
    def GetChannel(self, tag):
        ...
        if len(chname) != 2:
            raise SyntaxError(
                "Machine.GetChannel error: channel tag "+tag + " is invalid.")
        cname = chname[0]
        chname = chname[1]
        ...
        if self._IsGlobal(tag):
            ...
            if not (cname in list(circ.keys())):
                raise NameError(
                    "Machine.GetChannel error: circuit "+cname+" not found.")
            circ = circ[cname]
        ...
        if not(chname in list(allch.keys())):
            raise NameError(
                "Machine.GetChannel error: channel "+chname+" not found.")
    ...
    def GetOutputChannel(self, tag):
        ...
        if len(chname) != 2:
            raise SyntaxError(
                "Machine.GetOutputChannel error: channel tag "+tag + " is invalid.")
        cname = chname[0]
        chname = chname[1]
        ...
        if self._IsGlobal(tag):
            ...
            if not (cname in list(circ.keys())):
                raise NameError(
                    "Machine.GetOutputChannel error: circuit "+cname+" not found.")
            circ = circ[cname]
        ...
        if not(chname in list(allch.keys())):
            raise NameError(
                "Machine.GetOutputChannel error: channel "+chname+" not found.")
    ...
    def _IsGlobal(self, tag):
        ...
        if len(ctag) != 2:
            raise SyntaxError(
                "IsGlobal error: channel tag "+tag + " is invalid.")
    ...
    def Connect_OLD(self, *args):
        ...
        if self._IsGlobal(args[0]):
            ...
            print('PY: connect src - index ',
                  list(self.I.keys()).index(outsignal.name))
    ...
    def Connect(self, *args):
        ...
        for tag in args[1:]:  # for each target input tag
            ...
            Circuit.cCore.Connect(ccSrcID, ccSrcCH, metaSrc,
                                  ccDstID, ccDstCH, metaDst)
    ...
    def SetInput(self, channel=None, value=None):
        ...
        if ch.isInput == True:
            # find the position of the key
            idx = list(circ.I.keys()).index(key)
        else:
            idx = list(circ.O.keys()).index(key)
        print("PY: setinput "+circ.name+"."+key +
              ": "+str(value), circ.cCoreID, idx)
            ...
            if CIRCUIT not in CircuitExceptions:
                ...
                for Channel in channels:
                    if CIRCUIT+"."+Channel not in Exceptions:
                        ...
                        Circuit.cCore.SetInput(
                            circ.cCoreID, idx, c_double(value))
