...
class DFTD3(Circuit):
	...
	def __init__(self, machine, name, **keys):
		...
		Shiftxyz = [Tipx[LowestAtomIndex],Tipy[LowestAtomIndex],Tipz[LowestAtomIndex]]
		...
		while z <= (TipFinalZ-TipStartingXYZ[2]):
			...
			for i in range(0,NumberofTipAtoms+NumberofSurfaceAtoms):
				...
				f.write(str(TempAtoms[i])        + " "*(2-len(TempAtoms[i]))        + " "*(10+(12-len(str('{0:.10f}'.format(Tempx[i]) ) ) ))     + str('{0:.10f}'.format(Tempx[i]) )+   " "*(10+(12-len(str('{0:.10f}'.format(Tempy[i]) ) ) ))       + str('{0:.10f}'.format(Tempy[i]) ) + " "*10 + str('{0:.10f}'.format(Tempz[i]) )+ "\n")



			f.close()
			TipZArray.append(TipStartingXYZ[2]+z)
			fBash.write(str(DFTD3InstallPath)+"dftd3 " + 'z='+ str(TipStartingXYZ[2]+z) +".xyz" + " "+ options + " >"+str(TipStartingXYZ[2]+z)+".txt \n")
		...
		if RealitiveToSurface == True:
			for i in range(0,len(TipZArray)):
				FileO.write(str(TipZArray[i]-SurfaceHeight)+" "+str(Force[i])+ " "+str(Energy[i])+"\n")


		if RealitiveToSurface == False:
			for i in range(0,len(TipZArray)):
				FileO.write(str(TipZArray[i])+" "+str(Force[i])+ " "+str(Energy[i])+"\n")
