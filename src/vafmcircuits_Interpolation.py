...
class i3Dlin(Circuit):
    ...
    def __init__(self, machine, name, **keys):
        ...
        self.cCoreID = Circuit.cCore.Add_i3Dlin(
            self.machine.cCoreID, self.components)
        ...
        Circuit.cCore.i3Dlin_npts.restype = ctypes.POINTER(
    ...
    def Configure(self, **keys):
        ...
        if 'npoints' in list(keys.keys()):
            if len(keys['npoints']) != 3:
                raise ValueError(
                    "ERROR! the number of points is not a triplet!")
            else:
                self.npts = [int(n) for n in keys['npoints']]
                print("i3Dlin: npoints", self.npts)
                self.nptsSET = True
                # send the changes to cCore
                self.data = Circuit.cCore.i3Dlin_npts(
                    self.cCoreID, self.npts[0], self.npts[1], self.npts[2])
        ...
        if 'steps' in list(keys.keys()):
            if len(keys['steps']) != 3:
                ...
                Circuit.cCore.i3Dlin_step(
                    self.cCoreID, self.step[0], self.step[1], self.step[2])
        ...
        if 'pbc' in list(keys.keys()):
            if len(keys['pbc']) != 3:
                ...
                Circuit.cCore.i3Dlin_pbc(
                    self.cCoreID, self.pbc[0], self.pbc[1], self.pbc[2])
                self.pbcSET = True
    ...
    def ReadData(self, filename):
        ...
        if self.nptsSET == False:
            raise ValueError(
                "ERROR! The amount of grid points along each axis was not yet specified!")
        if self.stepSET == False:
            raise ValueError(
                "ERROR! The grid step sizes were not yet specified!")
...
class i1Dlin(Circuit):
    ...
    def __init__(self, machine, name, **keys):
        ...
        Circuit.cCore.Add_i1Dlin.argtypes = [
            ctypes.c_int, ctypes.c_int, ctypes.c_double, ctypes.c_int]
        self.cCoreID = Circuit.cCore.Add_i1Dlin(machine.cCoreID, ctypes.c_int(
            self.components), ctypes.c_double(step), ctypes.c_int(pbc))
    ...
    def SetData(self, datapoints):
        ...
        if(npts < 2):
            raise ValueError(
                "ERROR: there are less than 2 points in the field!")
...
class i4DlinVasp(Circuit):
    ...
    def __init__(self, machine, name, **keys):
        ...
        self.cCoreID = Circuit.cCore.Add_i4Dlin(
            machine.cCoreID, self.components)
        self.BiasStep = 0
        self.StartingV = -1
        self.pbcSET = False
        self.AddInput("x")
    ...
    def ConfigureVASP(self, **keys):
        ...
        if 'pbc' in list(keys.keys()):
            if len(keys['pbc']) != 4:
                ...
                Circuit.cCore.i4DLinPBC(
                    self.cCoreID, self.pbc[0], self.pbc[1], self.pbc[2], self.pbc[3])
                self.pbcSET = True
    ...
    def ReadVASPData(self, *filename):
            ...
            for linenumber, line in enumerate(f):
                ...
                if linenumber > NumberOfAtoms+9:
                    for i in range(0, (len(line.split()))):
                        ...
                        #
                        Density.append(
                            float(line.split()[i]) / (size[0]*size[1]*size[2]))
        ...
        # Set up Ctypes
        Circuit.cCore.i4Dlin_SetUpData.restype = ctypes.POINTER(
            ctypes.POINTER(ctypes.c_double))
        Circuit.cCore.i4Dlin_SetUpData.argtypes = [ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_int, ctypes.c_int,
                                                   ctypes.c_double,  ctypes.c_double,  ctypes.c_double,  ctypes.c_double,  ctypes.c_double]
        # Pass some data to C
        self.data = Circuit.cCore.i4Dlin_SetUpData(
            self.cCoreID, nx, ny, nz, nv, dx, dy, dz, dv,  self.StartingV)
...
class i4Dlin(Circuit):
    ...
    def Configure(self, **keys):
        ...
        if 'points' in list(keys.keys()):
            if len(keys['points']) != 4:
                raise ValueError(
                    "ERROR! the number of points requires 4 arguments! (nx,ny,nz,nv)")
            else:
                self.NPoints = [keys['points'][0], keys['points']
                                [1], keys['points'][2], keys['points'][3]]

        if 'step' in list(keys.keys()):
            if len(keys['step']) != 4:
                raise ValueError(
                    "ERROR! the step size requires 4 arguments! (nx,ny,nz,nv)")
            else:
                self.DX = keys['step'][0]
                self.DY = keys['step'][1]
                self.DZ = keys['step'][2]
    ...
    def ReadData(self, *filename):
        ...
        for line in f:
            ...
            for c in range(self.components):  # convert the components to float
                words[c+4] = ctypes.c_double(float(words[c+4])
                                             * self.ForceMultiplier)
    ...
    def __init__(self, machine, name, **keys):
        ...
        if FileExist == True and overide == False:
            print(
                "####################################################################################")
            print(
                "File already exists, skipping conversion, delete the file or change the filename")
            print(
                "####################################################################################")
        ...
        self.step = [round(sorted(set(Realx))[-1], 4) - sorted(set(Realx))[-2], round(sorted(set(
            Realy))[-1], 4) - sorted(set(Realy))[-2], round(sorted(Realz)[-1] - sorted(Realz)[-2], 4)]
        ...
        if FileExist == False or overide == True:
            ...
            Fx = [[[0 for k in range(self.MaxInd[2])] for j in range(
                self.MaxInd[1])] for i in range(self.MaxInd[0])]
            Fy = [[[0 for k in range(self.MaxInd[2])] for j in range(
                self.MaxInd[1])] for i in range(self.MaxInd[0])]
            Fz = [[[0 for k in range(self.MaxInd[2])] for j in range(
                self.MaxInd[1])] for i in range(self.MaxInd[0])]
            ...
            for FileName in files:
                if counter % 10 == 0:
                    print(
                        str(round(float(counter) / float(self.MaxInd[2]) * 100.0, 2)) + "% done")
            ...
            for X in range(0, self.MaxInd[0]):
                for Y in range(0, self.MaxInd[1]):
                    for Z in range(0, self.MaxInd[2]):
                        f.write(str(X+1) + " " + str(Y+1) + " " + str(Z+1 + offset) + " " + str(Fx[X][Y][self.MaxInd[2]-1 - Z]) + " " + str(
                            Fy[X][Y][self.MaxInd[2]-1 - Z]) + " " + str(Fz[X][Y][self.MaxInd[2]-1 - Z]) + "\n")
            f.close()
